...
-- Fri 02 Jun 2017 01:29:01 PM PDT
...
CREATE TABLE IF NOT EXISTS `foodtrac`.`Events` (
  ...
  INDEX `Events_Users` (`owner_id` ASC),
...
-- -----------------------------------------------------
-- Table `foodtrac`.`BrandAttendees`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `foodtrac`.`BrandAttendees` ;

CREATE TABLE IF NOT EXISTS `foodtrac`.`BrandAttendees` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `event_id` INT NOT NULL,
  `brand_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `BrandAttendees_Events` (`event_id` ASC),
  INDEX `BrandAttendees_Brands` (`brand_id` ASC),
  CONSTRAINT `BrandAttendees_Events`
    FOREIGN KEY (`event_id`)
    REFERENCES `foodtrac`.`Events` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `BrandAttendees_Brands`
    FOREIGN KEY (`brand_id`)
    REFERENCES `foodtrac`.`Brands` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT);
